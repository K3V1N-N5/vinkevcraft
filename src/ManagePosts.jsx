...
import { onAuthStateChanged, signOut } from 'firebase/auth'; // Tambahkan auth listener
import { auth, db, storage } from './firebase'; // Tambahkan auth, db, dan storage dari firebase
import { ref, uploadBytesResumable, getDownloadURL } from 'firebase/storage';
import { Button, TextInput, Textarea } from 'flowbite-react';
import { useNavigate } from 'react-router-dom';
import Login from './Login'; // Tambahkan komponen Login
...
function ManagePosts() {
  ...
  const [user, setUser] = useState(null); // State untuk user yang terautentikasi
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [isAddingOrEditing, setIsAddingOrEditing] = useState(false); // Kontrol visibilitas form
  ...
  // Fetch posts setelah user terautentikasi
  useEffect(() => {
    if (user) {
  ...
  const handleCreate = async (e) => {
    ...
    const newPost = {
      ...
      downloadLinks: form.downloadLinks
        ? form.downloadLinks.split(',').map((link) => {
